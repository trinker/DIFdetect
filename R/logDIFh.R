...
function(item, total, group, digits=3, output = "bare"){
    ...
    if (output == "full") {
        OP <- list("omnibus M3 to NULL"=anova(modn, mod2, test="Chisq"),
            ...
            "omnibus M1"=anova(modn, mod1, test="Chisq"), 
            "anova M1" = anova(mod1, test="Chisq"), 
            "betas M1" = TAB(mod1, digits=digits), 
            "omnibus M2"=anova(mod1, mod2, test="Chisq"), 
            "anova M2" = anova(mod2, test="Chisq"), 
            "betas M2" = TAB(mod2, digits=digits))
    } else {
        OP <- list("OMNIBUS BLOCK 1: DIF (INTERACTION to NULL)"
        	= anova(modn, mod2, test="Chisq"), 
            "OMNIBUS BLOCK 2: UNIFORM DIF (ADDIITIVE to NULL)" 
        	= anova(modn, mod1, test="Chisq"),  
            "betas BLOCK 2" = TAB(mod1, digits=digits), 
            "OMNIBUS BLOCK 3: NON-UNIFORM DIF (INTERACTION TO ADDITIVE)" 
